...
-- Fixes division leaders not seeing Awards in the mobile menu and RLS blocking award reads/writes.
-- If division leaders still see empty rosters/child lists, also run fix_division_leader_sports_roster_access.sql.
...
-- Quick check: division_leader awards permission (should return rows with can_access = true)
-- SELECT c.name, rp.role, rp.menu_item, rp.can_access
-- FROM public.role_permissions rp
-- JOIN public.companies c ON c.id = rp.company_id
-- WHERE rp.role = 'division_leader' AND rp.menu_item = 'awards'
-- ORDER BY c.name;
