...
-- Problem: policies required company_id = profiles.company_id while the app writes
-- currentCompany.id from the camp switcher — inserts could succeed but SELECT/RETURNING failed.
...
-- Verify: should return rows after adding notes in the app
-- SELECT id, admission_id, note, company_id, created_at
