-- =============================================================================
-- STEP 2 of 4 — APPLY FIX (run AFTER Step 1 backup is saved)
-- Todd-approved rules:
--   • Admin     → view + manage ALL incidents at their camp
--   • Div Leader / Viewer → view + add incidents for THEIR campers/division only
--   • Staff     → NO incidents access (no menu, blocked by RLS)
--   • Everyone  → their own camp only (no cross-camp)
-- Safe to re-run
-- =============================================================================
...
-- Edit / delete: Admin at their camp only
...
-- View all camp incidents: Admin only (NOT staff)
...
-- Create: Admin (all camp) or Division Leader / Viewer (scoped via child access)
...
-- Menu: Admin + Division Leader + Viewer YES; Staff NO
...
-- Sync login roles from profiles (fixes Deanna/Landon-style mismatches for Admin/DL)
...
-- Fix user_roles.company_id when profile camp differs.
-- Delete stale rows when the user already has a row at the correct company (avoids duplicate key).
...
-- Update remaining mismatches (safe: no duplicate user_id + company_id)
