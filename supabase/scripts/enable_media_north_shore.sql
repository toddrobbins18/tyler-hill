-- Enable Media menu for North Shore Day Camp roles (safe to re-run)
DO $$
DECLARE
  ns_company_id uuid;
BEGIN
  ...
  INSERT INTO public.role_permissions (company_id, role, menu_item, can_access) VALUES
    (ns_company_id, 'admin', 'media', true),
    (ns_company_id, 'super_admin', 'media', true),
    (ns_company_id, 'division_leader', 'media', true),
    (ns_company_id, 'staff', 'media', true)
