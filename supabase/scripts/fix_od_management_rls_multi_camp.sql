...
-- Run in Supabase SQL Editor. Same as migration 20260728130000_fix_od_management_rls_multi_camp.sql
...
ON public.bunks FOR SELECT TO authenticated
USING (company_id IS NOT NULL AND public.user_can_view_od_company_data(auth.uid(), company_id));

CREATE POLICY "Users can manage bunks for their company"
ON public.bunks FOR ALL TO authenticated
USING (company_id IS NOT NULL AND public.user_can_manage_od_data(auth.uid(), company_id))
WITH CHECK (company_id IS NOT NULL AND public.user_can_manage_od_data(auth.uid(), company_id));
...
ON public.bunk_staff FOR SELECT TO authenticated
USING (company_id IS NOT NULL AND public.user_can_view_od_company_data(auth.uid(), company_id));

CREATE POLICY "Users can manage bunk_staff for their company"
ON public.bunk_staff FOR ALL TO authenticated
USING (company_id IS NOT NULL AND public.user_can_manage_od_data(auth.uid(), company_id))
WITH CHECK (company_id IS NOT NULL AND public.user_can_manage_od_data(auth.uid(), company_id));
...
ON public.staff_days_off FOR SELECT TO authenticated
USING (company_id IS NOT NULL AND public.user_can_view_od_company_data(auth.uid(), company_id));

CREATE POLICY "Users can manage staff_days_off for their company"
ON public.staff_days_off FOR ALL TO authenticated
USING (company_id IS NOT NULL AND public.user_can_manage_od_data(auth.uid(), company_id))
WITH CHECK (company_id IS NOT NULL AND public.user_can_manage_od_data(auth.uid(), company_id));
