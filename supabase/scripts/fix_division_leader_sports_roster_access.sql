...
-- Fixes division leaders not seeing sports rosters / camper lists while admins can.
...
-- Audit: division leaders with campers in their bucket but 0 visible via RLS
WITH leaders AS (
  SELECT DISTINCT
    u.id AS user_id,
    u.email,
  ...
  JOIN public.user_roles ur
    ON ur.user_id = u.id
   AND ur.role = 'division_leader'::public.app_role
  JOIN public.staff s
    ON s.company_id = COALESCE(ur.company_id, p.company_id)
   AND lower(trim(COALESCE(s.email, ''))) = lower(trim(COALESCE(p.email, u.email)))
   AND s.division_id IS NOT NULL
   AND COALESCE(s.status, 'active') = 'active'
  JOIN public.divisions staff_div ON staff_div.id = s.division_id
  WHERE public.normalize_division_name_for_filter(staff_div.name) <> ''
)
SELECT
  l.email,
  l.staff_norm,
