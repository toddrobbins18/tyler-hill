...
import { probeCampminderCustomFields } from "../_shared/campminderCustomFields.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const person_id = String(body.person_id ?? "15899187");
    ...
    const probe = await probeCampminderCustomFields(
      person_id,
      season,
      authData.Token,
      subscriptionKey,
      clientId,
      body.expected_group ?? "Blue Jays",
    );
